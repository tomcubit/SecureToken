...
echo "trim():"
assert_eq "leading/trailing spaces" "abc" "$(trim '  abc  ')"
assert_eq "trailing CR"             "abc" "$(trim "$(printf 'abc\r')")"
assert_eq "trailing newline"        "abc" "$(trim "$(printf 'abc\n')")"
assert_eq "interior space KEPT"     "bad name" "$(trim ' bad name ')"
assert_eq "empty stays empty"       ""    "$(trim '')"
assert_eq "all-space -> empty"      ""    "$(trim '   ')"
